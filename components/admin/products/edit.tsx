...
interface ColorAndSizeObj {
  id: number;
  stock: number;
  additionalPrice: number;
}

const EditForm = ({ colors, sizes, categories, images, product }: Props) => {
  const [pending, setPending] = useState(false);
  const [subcategories, setSubcategories] = useState<
    CategoryController.SubCategory[]
  >([]);

  const productColors: ColorAndSizeObj[] = product.colors.map(color => {
    return {
      id: color.id,
      stock: color.ProductColor.stock,
      additionalPrice: Math.floor(Number(color.ProductColor.additionalPrice)),
    };
  });

  const productSizes: ColorAndSizeObj[] = product.sizes.map(size => {
    return {
      id: size.id,
      stock: size.ProductSize.stock,
      additionalPrice: Math.floor(Number(size.ProductSize.additionalPrice)),
    };
  });
  ...
  async function onSubmit(values: FormSchema) {
    setPending(true);
    const res = await updateProduct(product.id.toString(), values);
    if (res.success) {
  ...
  useEffect(() => {
    const array = product.categories.map(cat => {
      const category = categories.find(c => c.id === cat.id);
      return category ? category.subcategories : [];
    });
    setSubcategories(array.flat());
  }, []);
